...
fn test_distribute_usdc_success() {
    ...
    let result = client.distribute_usdc(&token_id, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&total), env.ledger().timestamp() + 1800), &accounts, &total);
...
fn test_distribute_usdc_emits_event() {
    ...
    client.initialize_split(&owner, &0, &token_id, &50, &30, &15, &5);
    let accounts = make_accounts(&env);
    client.distribute_usdc(&token_id, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
...
fn test_distribute_usdc_requires_auth() {
    ...
    client2.distribute_usdc(&token_id, &owner, &0, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&0), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
}
...
fn test_distribute_usdc_non_owner_rejected() {
    ...
    let accounts = make_accounts(&env);
    let result = client.try_distribute_usdc(&token_id, &attacker, &0, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&0), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
    assert_eq!(result, Err(Ok(RemittanceSplitError::Unauthorized)));
}
...
fn test_distribute_usdc_untrusted_token_rejected() {
    ...
    let accounts = make_accounts(&env);
    let result = client.try_distribute_usdc(&evil_token, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
    assert_eq!(
        result,
...
fn test_distribute_usdc_self_transfer_spending_rejected() {
    ...
    let result = client.try_distribute_usdc(&token_id, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
    assert_eq!(
        result,
...
fn test_distribute_usdc_zero_amount_rejected() {
    ...
    client.initialize_split(&owner, &0, &token_id, &50, &30, &15, &5);
    let accounts = make_accounts(&env);
    let result = client.try_distribute_usdc(&token_id, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&0), env.ledger().timestamp() + 1800), &accounts, &0);
    assert_eq!(result, Err(Ok(RemittanceSplitError::InvalidAmount)));
}
...
fn test_distribute_usdc_negative_amount_rejected() {
    ...
    client.initialize_split(&owner, &0, &token_id, &50, &30, &15, &5);
    let accounts = make_accounts(&env);
    let result = client.try_distribute_usdc(&token_id, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&-1), env.ledger().timestamp() + 1800), &accounts, &-1);
    assert_eq!(result, Err(Ok(RemittanceSplitError::InvalidAmount)));
}
...
fn test_distribute_usdc_not_initialized_rejected() {
    ...
    let accounts = make_accounts(&env);
    let result = client.try_distribute_usdc(&token_id, &owner, &0, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&0), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
    assert_eq!(result, Err(Ok(RemittanceSplitError::NotInitialized)));
}
...
fn test_distribute_usdc_replay_rejected() {
    ...
    let result = client.try_distribute_usdc(&token_id, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&500), env.ledger().timestamp() + 1800), &accounts, &500);
...
fn test_distribute_usdc_paused_rejected_and_unpause_restores_access() {
    ...
    let accounts = make_accounts(&env);
    let paused = client.try_distribute_usdc(&token_id, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
    assert_eq!(paused, Err(Ok(RemittanceSplitError::Unauthorized)));
...
fn test_distribute_usdc_rounding_remainder_goes_to_insurance() {
    ...
    client.initialize_split(&owner, &0, &token_id, &33, &33, &33, &1);
    let accounts = make_accounts(&env);
    client.distribute_usdc(&token_id, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&100), env.ledger().timestamp() + 1800), &accounts, &100);
...
fn test_distribute_usdc_multiple_rounds() {
    ...
    client.distribute_usdc(&token_id, &owner, &1, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&1), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
    client.distribute_usdc(&token_id, &owner, &2, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&2), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
    client.distribute_usdc(&token_id, &owner, &3, &(env.ledger().timestamp() + 1800), &RemittanceSplit::compute_request_hash(symbol_short!("distrib"), Address::generate(&env), *(&3), *(&1_000), env.ledger().timestamp() + 1800), &accounts, &1_000);
