...
fn test_get_all_goals_filters_by_owner() {
    ...
    let mut has_goal_a1 = false;
    let mut has_goal_a2 = false;
    let mut has_goal_a3 = false;
    for goal in goals_a.iter() {
        if goal.id == goal_a1 {
            has_goal_a1 = true;
        }
    ...
    assert!(has_goal_a1, "Goals for A should contain goal_a1");
    assert!(has_goal_a2, "Goals for A should contain goal_a2");
    assert!(has_goal_a3, "Goals for A should contain goal_a3");
    ...
    let mut has_goal_b1 = false;
    let mut has_goal_b2 = false;
    for goal in goals_b.iter() {
        if goal.id == goal_b1 {
            has_goal_b1 = true;
        }
    ...
    assert!(has_goal_b1, "Goals for B should contain goal_b1");
    assert!(has_goal_b2, "Goals for B should contain goal_b2");

    // Verify that goal IDs between owner_a and owner_b are disjoint
    for goal_a in goals_a.iter() {
        let goal_a_id = goal_a.id;
        assert!(
            goal_a_id != goal_b1 && goal_a_id != goal_b2,
