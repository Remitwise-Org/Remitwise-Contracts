...
The shared taxonomy ABI is pinned by tests in `remitwise-common/src/emit_tests.rs`.
Those tests assert:

- every `EventCategory` discriminant remains stable;
- every `EventPriority` discriminant remains stable;
- `RemitwiseEvents::emit` publishes the topic tuple
  `("Remitwise", category_u32, priority_u32, action)`;
- `RemitwiseEvents::emit_batch` publishes the fixed batch topic tuple
  `("Remitwise", category_u32, EventPriority::Low, "batch")` with payload
  `(action, count)`, including a zero-count batch.
