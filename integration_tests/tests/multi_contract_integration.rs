...
use soroban_sdk::{testutils::{Address as _, Events}, Address, Env, String as SorobanString, Symbol, TryFromVal};
...
fn test_multi_contract_user_flow() {
    ...
    let nonce = 0u64;
    remittance_client.initialize_split(&user, &nonce, &usdc_id, &40u32, &30u32, &20u32, &10u32);
...
fn test_multiple_entities_creation() {
    ...
    let policy1 = insurance_client.create_policy(
        ...
        &SorobanString::from_str(&env, "life"), &150i128, &100_000i128, &None,
    );
    assert_eq!(policy1, 1u32);

    let policy2 = insurance_client.create_policy(
        &user, &SorobanString::from_str(&env, "Emergency Coverage"),
        &SorobanString::from_str(&env, "emergency"), &50i128, &10_000i128, &None,
    );
