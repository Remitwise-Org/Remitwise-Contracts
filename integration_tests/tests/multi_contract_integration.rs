...
use soroban_sdk::{
    ...
    Address, Env, String as SorobanString, Symbol, TryFromVal,
};
...
impl ExpectedTotals {
    fn record(&mut self, split: SplitAmounts, total: i128) {
        self.remitted += total;
        self.spending += split.spending;
        self.savings += split.savings;
        self.bills += split.bills;
        self.insurance += split.insurance;
    }
}
...
fn test_multi_contract_user_flow() {
    ...
    let user = Address::generate(&env);
    let trusted_usdc = Address::generate(&env);
...
fn test_split_with_rounding() {
    ...
    remittance_client.initialize_split(&user, &0u64, &trusted_usdc, &33u32, &33u32, &17u32, &17u32);
...
fn test_integration_accounting_split_sums_to_total() {
    ...
    remittance_client.initialize_split(&user, &0u64, &trusted_usdc, &40u32, &30u32, &20u32, &10u32);
...
impl TestHarness {
    /// @notice Deploys the contracts used by the cross-contract invariant suite.
    /// @dev The same owner is used across all contracts so aggregate accounting
    ///      totals can be compared without filtering ambiguity.
    fn new(spending: u32, savings: u32, bills: u32, insurance: u32) -> Self {
        let env = Env::default();
        env.mock_all_auths();
        ...
        let goal_id = SavingsGoalContractClient::new(&env, &savings_id).create_goal(
            &user,
            &SorobanString::from_str(&env, "Primary Goal"),
            &100_000i128,
    ...
    fn remittance_client(&self) -> RemittanceSplitClient<'_> {
        RemittanceSplitClient::new(&self.env, &self.remittance_id)
    }

    fn savings_client(&self) -> SavingsGoalContractClient<'_> {
        SavingsGoalContractClient::new(&self.env, &self.savings_id)
    }

    fn bills_client(&self) -> BillPaymentsClient<'_> {
        BillPaymentsClient::new(&self.env, &self.bills_id)
    }

    fn insurance_client(&self) -> InsuranceClient<'_> {
        InsuranceClient::new(&self.env, &self.insurance_id)
    }

    /// @notice Converts the vector returned by `calculate_split` into named fields.
    /// @custom:security This assertion defends against accidental category reordering.
    fn calculate_split(&self, total: i128) -> SplitAmounts {
        let amounts = self.remittance_client().calculate_split(&total);
        assert_eq!(amounts.len(), 4);

        SplitAmounts {
            spending: amounts.get(0).unwrap(),
            savings: amounts.get(1).unwrap(),
            bills: amounts.get(2).unwrap(),
            insurance: amounts.get(3).unwrap(),
        }
    }

    /// @notice Applies the non-spending allocation to the downstream contracts.
    /// @dev Bills are created as non-recurring so unpaid totals remain a direct
    ///      representation of the allocated amount until an explicit payment.
    fn apply_downstream_allocation(&self, split: SplitAmounts, sequence: u32) -> (u32, u32) {
        self.savings_client()
            .add_to_goal(&self.user, &self.goal_id, &split.savings);

        let bill_id = self.bills_client().create_bill(
            &self.user,
            &SorobanString::from_str(&self.env, "Invariant Bill"),
            &split.bills,
            &(self.env.ledger().timestamp() + ((sequence as u64 + 1) * 86_400)),
            &false,
            &0u32,
            &SorobanString::from_str(&self.env, "USDC"),
        );

        let policy_id = self.insurance_client().create_policy(
            &self.user,
            &SorobanString::from_str(&self.env, "Invariant Policy"),
            &SorobanString::from_str(&self.env, "health"),
            &split.insurance,
            &(split.insurance * 12),
        );
        ...
        let bill = self
            .bills_client()
            .get_bill(&bill_id)
            .expect("bill should exist after creation");
        let policy = self
            .insurance_client()
            .get_policy(&policy_id)
            .expect("policy should exist after creation");

        assert_eq!(bill.amount, split.bills);
        assert_eq!(policy.monthly_premium, split.insurance);

        (bill_id, policy_id)
    }

    fn goal_balance(&self) -> i128 {
        self.savings_client()
    ...
    fn unpaid_bills_total(&self) -> i128 {
        self.bills_client().get_total_unpaid(&self.user)
    }

    fn active_premiums_total(&self) -> i128 {
        self.insurance_client()
            .get_total_monthly_premium(&self.user)
    }

    fn downstream_total(&self) -> i128 {
        self.goal_balance() + self.unpaid_bills_total() + self.active_premiums_total()
    }
}

/// @notice Verifies one remittance remains lossless across all allocation buckets.
/// @custom:security Prevents silent divergence between split calculation and the
///                  downstream contract amounts recorded for the same remittance.
#[test]
fn test_single_remittance_allocation_sum_consistency() {
    let harness = TestHarness::new(40, 30, 20, 10);
    let total = 10_000i128;

    let split = harness.calculate_split(total);
    harness.apply_downstream_allocation(split, 0);

    assert_eq!(split.spending, 4_000);
    assert_eq!(split.savings, 3_000);
    assert_eq!(split.bills, 2_000);
    assert_eq!(split.insurance, 1_000);
    assert_eq!(
        split.spending + split.savings + split.bills + split.insurance,
        total
    );

    assert_eq!(harness.goal_balance(), split.savings);
    assert_eq!(harness.unpaid_bills_total(), split.bills);
    assert_eq!(harness.active_premiums_total(), split.insurance);
    assert_eq!(harness.downstream_total(), total - split.spending);
}

/// @notice Verifies cumulative downstream totals track multiple remittances exactly.
/// @dev This test records the spending bucket separately because only savings,
///      bills, and insurance are represented in downstream contract state.
#[test]
fn test_multiple_remittances_preserve_cumulative_downstream_totals() {
    let harness = TestHarness::new(40, 30, 20, 10);
    let remittances = [10_000i128, 17_333i128, 6_512i128];
    let mut expected = ExpectedTotals::default();

    for (index, total) in remittances.iter().enumerate() {
        let split = harness.calculate_split(*total);
        harness.apply_downstream_allocation(split, index as u32);
        expected.record(split, *total);
    }

    assert_eq!(
        expected.spending + expected.savings + expected.bills + expected.insurance,
        expected.remitted
    );
    assert_eq!(harness.goal_balance(), expected.savings);
    assert_eq!(harness.unpaid_bills_total(), expected.bills);
    assert_eq!(harness.active_premiums_total(), expected.insurance);
    assert_eq!(
        harness.downstream_total() + expected.spending,
        expected.remitted
    );
}

/// @notice Verifies the insurance bucket absorbs integer rounding remainder.
/// @custom:security Guards against precision loss when split percentages do not
///                  divide evenly into the remittance amount.
#[test]
fn test_rounding_remainder_is_assigned_without_sum_loss() {
    let harness = TestHarness::new(33, 33, 17, 17);
    let total = 101i128;

    let split = harness.calculate_split(total);
    harness.apply_downstream_allocation(split, 0);

    assert_eq!(split.spending, 33);
    assert_eq!(split.savings, 33);
    assert_eq!(split.bills, 17);
    assert_eq!(split.insurance, 18);
    assert_eq!(
        split.spending + split.savings + split.bills + split.insurance,
        total
    );
...
/// @notice Verifies downstream aggregate totals update after state transitions.
/// @dev A paid bill must leave unpaid totals, and a deactivated policy must
///      leave active premium totals, without altering the savings ledger.
#[test]
fn test_net_downstream_totals_follow_bill_payment_and_policy_deactivation() {
    let harness = TestHarness::new(40, 30, 20, 10);
    let total = 8_400i128;

    let split = harness.calculate_split(total);
    let (bill_id, policy_id) = harness.apply_downstream_allocation(split, 0);

    assert_eq!(harness.goal_balance(), split.savings);
    assert_eq!(harness.unpaid_bills_total(), split.bills);
    assert_eq!(harness.active_premiums_total(), split.insurance);

    harness.bills_client().pay_bill(&harness.user, &bill_id);
    assert_eq!(harness.goal_balance(), split.savings);
    assert_eq!(harness.unpaid_bills_total(), 0);
    assert_eq!(harness.active_premiums_total(), split.insurance);

    harness
        .insurance_client()
        .deactivate_policy(&harness.user, &policy_id);
    assert_eq!(harness.goal_balance(), split.savings);
    assert_eq!(harness.unpaid_bills_total(), 0);
    assert_eq!(harness.active_premiums_total(), 0);
    assert_eq!(harness.downstream_total(), split.savings);
}

/// Event topic compliance for contracts that already use the shared emitter.
///
/// `bill_payments` emits via `remitwise_common::RemitwiseEvents`, so its topics
/// should follow the deterministic schema:
/// `("Remitwise", category: u32, priority: u32, action: Symbol)`.
#[test]
fn test_bill_payments_event_topic_compliance() {
    ...
    assert!(
        events.len() > 0,
        "No bill_payments events were emitted by the sample action"
    );
    ...
    for ev in events.iter() {
        ...
        let ok = topics.len() == 4
            && Symbol::try_from_val(&env, &topics.get(0).unwrap())
                .map(|topic| topic == symbol_short!("Remitwise"))
                .unwrap_or(false);
        if !ok {
    ...
    assert_eq!(
        non_compliant.len(),
        0u32,
        "Found bill_payments events that do not follow the Remitwise topic schema."
    );
}
