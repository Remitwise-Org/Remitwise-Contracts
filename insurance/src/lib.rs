...
/// Instance storage TTL bump amount (30 days in ledgers).
const INSTANCE_BUMP_AMOUNT: u32 = 518_400;

/// Instance storage TTL threshold before bump (1 day in ledgers).
...
pub enum DataKey {
    ...
    ActivePolicies,
    /// Global schedule counter (u32).
    ScheduleCount,
    /// Individual premium schedule keyed by its u32 ID.
    Schedule(u32),
    /// List of all active schedule IDs.
    ActiveSchedules,
}
...
/// A premium payment schedule for automated recurring payments.
#[contracttype]
#[derive(Clone, Debug)]
pub struct PremiumSchedule {
    /// Unique monotonic ID assigned at creation.
    pub id: u32,
    /// The policy ID this schedule is associated with.
    pub policy_id: u32,
    /// Owner address of the schedule.
    pub owner: Address,
    /// Next due timestamp for this schedule.
    pub next_due: u64,
    /// Interval in seconds between payments (0 = one-time).
    pub interval: u64,
    /// Whether this schedule is active.
    pub active: bool,
    /// Count of missed payment periods.
...
impl InsuranceContract {
    ...
    /// Creates a new premium payment schedule for a policy.
    ///
    /// # Arguments
    ///
    /// * `caller` - Address of the schedule owner (must sign).
    /// * `policy_id` - ID of the policy this schedule is for.
    /// * `next_due` - Timestamp when the first payment is due.
    /// * `interval` - Seconds between payments (0 = one-time schedule).
    ///
    /// # Returns
    ///
    /// The newly-assigned schedule ID (`u32`).
    pub fn create_premium_schedule(
        ...
        let mut count: u32 = env
            ...
            .unwrap_or(0u32);
        count = count.checked_add(1).expect("schedule id overflow");
        ...
        let mut active_ids: Vec<u32> = env
            .storage()
            .instance()
            .get(&DataKey::ActiveSchedules)
            .unwrap_or(Vec::new(&env));
        active_ids.push_back(schedule_id);
        env.storage()
            .instance()
            .set(&DataKey::ActiveSchedules, &active_ids);

        schedule_id
    }

    /// Modifies an existing premium schedule.
    ///
    /// # Arguments
    ///
    /// * `caller` - Must be the schedule owner.
    /// * `schedule_id` - ID of the schedule to modify.
    /// * `next_due` - New next due timestamp.
    ...
    pub fn modify_premium_schedule(
        ...
        let mut schedule: PremiumSchedule = env
            .storage()
            .instance()
    ...
    /// Cancels an active premium schedule.
    ///
    /// # Arguments
    ///
    /// * `caller` - Must be the schedule owner.
    ...
    pub fn cancel_premium_schedule(env: Env, caller: Address, schedule_id: u32) {
        ...
        env.storage()
            .instance()
            .set(&DataKey::ActiveSchedules, &new_ids);
    }

    /// Executes all due premium schedules.
    ///
    /// Iterates through active schedules and processes any that are due.
    /// For recurring schedules, advances the next_due by the interval.
    /// For one-time schedules (interval = 0), deactivates after execution.
    ///
    /// # Returns
    ///
    /// A vector of schedule IDs that were executed.
    pub fn execute_due_premium_schedules(env: Env) -> Vec<u32> {
        ...
        let mut remaining_active: Vec<u32> = Vec::new(&env);

        for schedule_id in active_ids.iter() {
            let mut schedule: PremiumSchedule = env
                .storage()
                .instance()
                .get(&DataKey::Schedule(schedule_id))
            ...
            if now >= schedule.next_due {
                let mut policy: Policy = env
                    .storage()
                    .instance()
                    .get(&DataKey::Policy(schedule.policy_id))
                    .unwrap_or_else(|| panic!("policy not found"));

                if policy.active {
                    policy.last_payment_at = now;
                    policy.next_payment_due = now.saturating_add(PREMIUM_INTERVAL_SECONDS);
                    env.storage()
                        .instance()
                ...
                if schedule.interval > 0 {
                    let mut missed = 0u32;
                    let mut new_due = schedule.next_due.saturating_add(schedule.interval);
                    while new_due <= now {
                        missed = missed.saturating_add(1);
                        new_due = new_due.saturating_add(schedule.interval);
                    }
                    schedule.next_due = new_due;
                    schedule.missed_count = schedule.missed_count.saturating_add(missed);
                    env.storage()
                        .instance()
                        .set(&DataKey::Schedule(schedule_id), &schedule);
                    remaining_active.push_back(schedule_id);
                } else {
                    schedule.active = false;
                    env.storage()
                        .instance()
        ...
        env.storage()
            .instance()
            .set(&DataKey::ActiveSchedules, &remaining_active);

        executed
    }

    /// Returns a premium schedule by ID.
    ...
    /// Returns all active schedule IDs.
    pub fn get_active_schedules(env: Env) -> Vec<u32> {
        Self::assert_initialized(&env);
        env.storage()
            .instance()
            .get(&DataKey::ActiveSchedules)
    ...
    /// Extends the instance storage TTL if below threshold.
