...
impl Insurance {
    ...
    pub fn set_pause_admin(env: Env, caller: Address, new_admin: Address) -> Result<(), InsuranceError> {
        caller.require_auth();
        ...
        env.storage().instance().set(&symbol_short!("PAUSE_ADM"), &new_admin);
        Ok(())
    }
    ...
    pub fn pause(env: Env, caller: Address) -> Result<(), InsuranceError> {
        ...
        env.events().publish((symbol_short!("insure"), symbol_short!("paused")), ());
        Ok(())
    }
    ...
    pub fn unpause(env: Env, caller: Address) -> Result<(), InsuranceError> {
        ...
        env.events().publish((symbol_short!("insure"), symbol_short!("unpaused")), ());
        Ok(())
    }
    ...
    pub fn pause_function(env: Env, caller: Address, func: Symbol) -> Result<(), InsuranceError> {
        ...
        let mut m: Map<Symbol, bool> = env
            ...
            .unwrap_or_else(|| Map::new(&env));
        m.set(func, true);
        env.storage().instance().set(&symbol_short!("PAUSED_FN"), &m);
        Ok(())
    }
    ...
    pub fn set_upgrade_admin(env: Env, caller: Address, new_admin: Address) -> Result<(), InsuranceError> {
        caller.require_auth();
        ...
        env.storage().instance().set(&symbol_short!("UPG_ADM"), &new_admin);
        Ok(())
    }
    ...
    /// Add tags to a policy, skipping duplicates.
    ...
    /// Remove tags from a policy. Tags not present are silently ignored.
    ...
    pub fn batch_pay_premiums(
        ...
        env.storage().instance().set(&symbol_short!("POLICIES"), &policies_map);
        env.events().publish(
    ...
    pub fn get_active_policies(env: Env, owner: Address, cursor: u32, limit: u32) -> PolicyPage {
        ...
        // If we didn't fill the page, no more results
        if count < limit {
            next_cursor = 0;
        }
    ...
    pub fn create_premium_schedule(
        ...
        env.storage().instance().set(&symbol_short!("PREM_SCH"), &schedules);
        env.storage().instance().set(&symbol_short!("NEXT_PSCH"), &next_schedule_id);
    ...
    pub fn modify_premium_schedule(
        ...
        let mut schedule = schedules.get(schedule_id).ok_or(InsuranceError::PolicyNotFound)?;
