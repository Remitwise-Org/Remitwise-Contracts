...
mod test_events {
    ...
    fn test_get_active_policies_multi_owner_isolation() {
        ...
        let name = String::from_str(&env, "Health");
        let coverage_type = CoverageType::Health;
        
        client.create_policy(&owner_a, &name, &coverage_type, &100, &10000, &None);
        client.create_policy(&owner_b, &name, &coverage_type, &100, &10000, &None);
    ...
    fn test_deactivate_policy_emits_event() {
        ...
        // Deactivate policy
        let result = client.deactivate_policy(&owner, &policy_id).unwrap();
        assert!(result);
    ...
    fn test_create_policy_emits_event_exists() {
        ...
        let external_ref = Some(String::from_str(&env, "EXT-1"));

        client.create_policy(&owner, &name, &coverage_type, &monthly_premium, &coverage_amount, &external_ref);
