...
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SchedulingControlEvent {
    pub version: u32,
    pub seq: u64,
    pub kind: Symbol,
...
impl BillPayments {
    ...
    fn next_schedule_event_seq(env: &Env) -> u64 {
        let prev: u64 = env
            .storage()
            .instance()
        ...
        env.storage().instance().set(&symbol_short!("SCH_SEQ"), &next);
        next
    }

    fn emit_scheduling_control_event(env: &Env, kind: Symbol, actor: Option<&Address>, timestamp: u64) {
        let seq = Self::next_schedule_event_seq(env);
        env.events().publish(
            (symbol_short!("scheduling"), symbol_short!("control")),
            SchedulingControlEvent {
    ...
    fn create_bill_schedule_core(
        ...
        Self::emit_scheduling_control_event(env, symbol_short!("cre_sch"), Some(&owner), current_time);
    ...
    fn modify_bill_schedule_core(
        ...
        Self::emit_scheduling_control_event(env, symbol_short!("mod_sch"), Some(&caller), current_time);
    ...
    fn cancel_bill_schedule_core(
        ...
        Self::emit_scheduling_control_event(env, symbol_short!("can_sch"), Some(&caller), current_time);
    ...
    fn execute_due_bill_schedules_core(env: &Env, actor: Option<&Address>) -> Vec<u32> {
        ...
        if !executed.is_empty() {
            Self::emit_scheduling_control_event(env, symbol_short!("exe_sch"), actor, current_time);
        }
