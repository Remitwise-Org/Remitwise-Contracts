...
/// Regression: multiple consecutive page requests with same cursor return identical results.
#[test]
fn test_cursor_idempotence() {
    ...
    let page1 = client.get_archived_bills_page(&owner, &3, &5);
    let page2 = client.get_archived_bills_page(&owner, &3, &5);
    let page3 = client.get_archived_bills_page(&owner, &3, &5);
    
    assert_eq!(page1.count, page2.count);
    assert_eq!(page1.count, page3.count);
    assert_eq!(page1.next_cursor, page2.next_cursor);
    assert_eq!(page1.next_cursor, page3.next_cursor);
    
    let ids1: Vec<u32> = page1.items.iter().map(|b| b.id).collect();
    let ids2: Vec<u32> = page2.items.iter().map(|b| b.id).collect();
    let ids3: Vec<u32> = page3.items.iter().map(|b| b.id).collect();
    
    assert_eq!(ids1, ids2);
    assert_eq!(ids1, ids3);
}

/// Regression: restore middle items creates gaps but pagination still returns correct sequence.
#[test]
fn test_restore_creates_gaps_pagination_handles_correctly() {
    let env = make_env();
    let (client, owner) = setup_client(&env);
    create_pay_archive(&env, &client, &owner, 10);
    ...
    // Restore bills 3, 5, 7 - creates gaps in the middle
    client.restore_bill(&owner, &initial[2]);
    client.restore_bill(&owner, &initial[4]);
    client.restore_bill(&owner, &initial[6]);
    
    let after = paginate_all(&client, &owner, 3);
    assert_eq!(after.len(), 7, "should have 7 bills after removing 3");
    
    // Verify no duplicates
    let mut deduped = after.clone();
    deduped.sort();
    deduped.dedup();
    assert_eq!(deduped.len(), after.len());
    
    // Verify removed bills don't appear
    assert!(!after.contains(&initial[2]));
    assert!(!after.contains(&initial[4]));
    assert!(!after.contains(&initial[6]));
}

/// Regression: archiving bills after partial pagination doesn't affect ongoing iteration.
#[test]
fn test_archive_during_pagination_isolation() {
    let env = make_env();
    let (client, owner) = setup_client(&env);
    create_pay_archive(&env, &client, &owner, 5);
    
    // Start pagination
    let page1 = client.get_archived_bills_page(&owner, &0, &3);
    ...
    // Continue with saved cursor - should still work
    let page2 = client.get_archived_bills_page(&owner, &saved_cursor, &3);
    
    // Should see at least the remaining original bills
    assert!(page2.count >= 2, "should see remaining original bills");
}

/// Regression: limit at exact page boundaries produces no off-by-one errors.
#[test]
fn test_exact_page_boundaries_no_off_by_one() {
    let env = make_env();
    let (client, owner) = setup_client(&env);
    create_pay_archive(&env, &client, &owner, 12);
    
    // Request exactly 4 items per page, should produce 3 pages
    let page1 = client.get_archived_bills_page(&owner, &0, &4);
    assert_eq!(page1.count, 4);
    ...
    let page3 = client.get_archived_bills_page(&owner, &page2.next_cursor, &4);
    assert_eq!(page3.count, 4);
    assert_eq!(page3.next_cursor, 0, "third page should be last");
    
    // Verify all items unique
    let mut all_ids = Vec::new();
    for bill in page1.items.iter() { all_ids.push(bill.id); }
    for bill in page2.items.iter() { all_ids.push(bill.id); }
    for bill in page3.items.iter() { all_ids.push(bill.id); }
    
    let mut deduped = all_ids.clone();
    deduped.sort();
    deduped.dedup();
    assert_eq!(deduped.len(), 12, "must have exactly 12 unique items");
}

/// Regression: bulk cleanup after saving cursor invalidates cursor gracefully.
#[test]
fn test_bulk_cleanup_mid_pagination_safe() {
    let env = make_env();
    let (client, owner) = setup_client(&env);
    create_pay_archive(&env, &client, &owner, 10);
    ...
    // Using stale cursor should return empty page, not panic
    let page2 = client.get_archived_bills_page(&owner, &cursor, &4);
