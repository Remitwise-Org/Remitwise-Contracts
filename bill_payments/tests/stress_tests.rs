...
fn stress_200_bills_single_user() {
    ...
    for _ in 0..200 {
        client.create_bill(&owner, &name, &100i128, &due_date, &false, &0u32, &None, &soroban_sdk::String::from_str(&env, "XLM"));
    }
...
fn stress_bills_across_10_users() {
    ...
    for user in &users {
        for _ in 0..BILLS_PER_USER {
            client.create_bill(user, &name, &AMOUNT_PER_BILL, &due_date, &false, &0u32, &None, &soroban_sdk::String::from_str(&env, "XLM"));
        }
    }
...
fn stress_ttl_re_bumped_by_pay_bill_after_ledger_advancement() {
    ...
    let bill_id = client.create_bill(&owner, &name, &500i128, &due_date, &false, &0u32, &None, &soroban_sdk::String::from_str(&env, "XLM"));
...
fn stress_archive_100_paid_bills() {
    ...
    for _ in 0..100 {
        client.create_bill(&owner, &name, &200i128, &due_date, &false, &0u32, &None, &soroban_sdk::String::from_str(&env, "XLM"));
    }
...
fn stress_archive_across_5_users() {
    ...
    for (i, user) in users.iter().enumerate() {
        let first = next_id;
        for _ in 0..BILLS_PER_USER {
            client.create_bill(user, &name, &100i128, &due_date, &false, &0u32, &None, &soroban_sdk::String::from_str(&env, "XLM"));
            next_id += 1;
        }
        let last = next_id - 1;
